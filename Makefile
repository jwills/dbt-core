...
.PHONY: unit
unit: .env ## Runs unit tests with py39.
	@\
	$(DOCKER_CMD) tox -e py39

.PHONY: test
test: .env ## Runs unit tests with py39 and code checks against staged changes.
	@\
	$(DOCKER_CMD) tox -e py39; \
...
.PHONY: integration
integration: .env ## Runs postgres integration tests with py39.
	@\
	$(DOCKER_CMD) tox -e py39-integration -- -nauto

.PHONY: integration-fail-fast
integration-fail-fast: .env ## Runs postgres integration tests with py39 in "fail fast" mode.
	@\
	$(DOCKER_CMD) tox -e py39-integration -- -x -nauto
